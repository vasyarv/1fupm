...
1. он мультиплатформенный, т.е. для портирования программы на разные ОС достаточно только изменить код взаимодействия с системой, а всю программу переписывать не нужно

2. его синтаксис непротиворечив и недвусмысленен

3. он имеет привычный Intel-синтаксис, т.е. программист на MASM или TASM сможет без особых проблем перейти на NASM
...
###Работа с printf/scanf
В случае использования данных функций лучше использовать `gcc` вместо `ld` при линковке. Он автоматически подцепит стандартные функции.
Тогда **в коде программы нужно исправить _start на main** т.к. gcc ждет именно его.

В случае компиляции и линковки 32-битного кода делаем так:
...
`times` заставляет выполняться инструкции заданное число раз (в секции data)

http://www.nasm.us/doc/nasmdoc3.html#section-3.2.5
